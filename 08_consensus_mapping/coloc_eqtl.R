...
result_cols <- c(c('Prefix','Locus','Tissue','Gene','Gene_id','TSS_distance',
...
#remove duplicates
input <- input %>%
  arrange(SNP, gene_id, pval_nominal) %>%
  group_by(SNP, gene_id) %>%
  slice_head(n = 1) %>%
  ungroup()
input <- as.data.frame(input)

gene_lst <- unique(input$gene_id)
for (gene in gene_lst)
{
  temp <- input[which(input$gene_id == gene),]
  gene_name <- temp[1,'gene_name']
  if (type=='cc'){
  ...
  c_temp <- list(Prefix=prefix,Locus=locus_name,Tissue=tissue,REF=ref,ALT=alt,BETA_QTL=beta_qtl,
                 Gene=gene_name,Gene_id=gene,TSS_distance=tss,SNP=lead,P_GWA=p_gwa,P_QTL=p_qtl,SNP.PP.H4=snp_h4,
